describe("1. Auth & Profile E2E Tests (Khang's Module - 14 cases)", () => {
  ...
  // -------------------------------------------------------------
  // TC5: Text quá tải (>255 ký tự)
  // -------------------------------------------------------------
  it("TC5: Register name length exceeds maximum limit (255 characters)", () => {
    cy.visit("/register");
    ...
    cy.get('form input[type="text"]').eq(0).type("a".repeat(260));
    ...
    cy.get('form button[type="submit"]').click();
    cy.get("form").should(($form) => {
      expect($form.text().toLowerCase()).to.match(/tên|vượt quá|tối đa|255/i);
    });
  });
