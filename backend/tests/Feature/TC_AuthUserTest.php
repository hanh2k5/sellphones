...
class TC_AuthUserTest extends TestCase
{
    ...
    public function test_TC1_delete_user_already_deleted()
    {
        ...
        $this->withToken($this->adminToken)->deleteJson("/api/admin/users/{$id}")->assertStatus(404);
    }
    ...
    {
        // Đăng nhập sai 5+ lần → phải bị khóa tạm
        $email = 'user@test.com';
        for ($i = 0; $i < 5; $i++) {
            $this->postJson('/api/login', ['email' => $email, 'password' => 'WrongPass!']);
        }

        // Lần thứ 6 → phải bị chặn (429 Too Many Requests hoặc 422 với thông báo khóa)
        $response = $this->postJson('/api/login', [
            'email'    => $email,
            'password' => 'WrongPass!',
        ]);

        $this->assertContains($response->getStatusCode(), [429, 422, 401],
            'Brute Force: Sau 5 lần sai phải bị khóa tạm (429/422)');
    }
    ...
    public function test_TC14_delete_user_with_user_token_forbidden()
    {
        ...
        $this->withToken($this->userToken)->deleteJson("/api/admin/users/{$target->id}")->assertStatus(403);
        $this->assertDatabaseHas('users', ['id' => $target->id]);
    }

    public function test_TC14_admin_cannot_delete_himself()
    {
